var loadPage = function(companyName) {
    ...
    var plotMetrics = function() {
        ...
        var getCompanyDataset = function(metric, dataset) {
            var hd = dataset.filter(function(d) { return d.name === "Headcount" })
                          .map(function(d) { return d.value });
            var res = new Array();
            if (metric === "Revenue/Headcount") {
                var rev = dataset.filter(function(d) { return d.name === "Revenue" })
                          .map(function(d) { return d.value });
                ...
                var cashBurn = dataset.filter(function(d) { return d.name === "Cash Burn" })
                          .map(function(d) { return d.value });
                for (var i=0;i<cashBurn.length;i++) {
                    res.push(cashBurn[i]/hd[i]);
                }
                return res;
            }
            return dataset.filter(function(d) { return d.name === metric })
        ...
        METRICS.forEach(function(m) {
            ...
            //var data = [_.uniq(filterDates(m, model.metrics))];
